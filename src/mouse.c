...
void print_mouse_info() {
    draw_string(0, 5, COLOR_BLACK, "Mouse Click to change colors");
    ...
    if (g_status.left_button) {
        if (back_color < COLOR_WHITE)
            back_color++;
    }
    if (g_status.right_button) {
        if (back_color > COLOR_BLACK)
            back_color--;
    }
}
...
void mouse_handler(REGISTERS *r) {
    ...
    switch (mouse_cycle) {
        ...
        case 2:
            ...
            vga_graphics_draw_rect(g_mouse_x_pos, g_mouse_y_pos, 10, 10, COLOR_BLUE);
            print_mouse_info();
            mouse_cycle = 0;
            break;
    }
